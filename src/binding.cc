...
void QueryAfter(uv_work_t* req) {
  ...
  if (query_req->error_count == query_req->errors.size()) {
    ...
    for (size_t i = 0; i < query_req->results.size(); ++i) {
      //~ if (query_req->error_count && query_req->errors[i]) {
...
/*NAN_METHOD(DBHandle::QuerySync) {
  DBHandle* self = Nan::ObjectWrap::Unwrap<DBHandle>(info.Holder());

  if (!self->db_)
    return Nan::ThrowError("Database not open");

  Nan::Utf8String sql_str(info[0]);
  uint32_t prepare_flags = Nan::To<uint32_t>(info[1]).FromJust();
  uint32_t query_flags = Nan::To<uint32_t>(info[2]).FromJust();

  NamedParamsMap* bind_map = nullptr;
  vector<BindValue>* bind_list = nullptr;
  if (info[3]->IsArray()) {
    Local<Array> param_list = Local<Array>::Cast(info[3]);
    if (query_flags & QueryFlag::NamedParams) {
      // [ key1, val1, key2, val2, ... ]
      bind_map = new NamedParamsMap();
      for (uint32_t i = 0; i < param_list->Length(); i += 2) {
        BindValue bv;
        Local<Value> js_key = Nan::Get(param_list, i).ToLocalChecked();
        Nan::Utf8String key_str(js_key);
        ...
        bind_map->emplace(make_pair(string(*key_str, key_str.length()), bv));
      }
    } else {
      // [ val1, val2, .... ]
      bind_list = new vector<BindValue>(param_list->Length());
      for (uint32_t i = 0; i < param_list->Length(); ++i) {
        Local<Value> js_val = Nan::Get(param_list, i).ToLocalChecked();
        if (!set_bind_value(bind_list->at(i), js_val)) {
          delete bind_list;
          string msg = "Unsupported value for bind parameter at position ";
          msg += to_string(i);
          msg += ": ";
  ...
  Local<Function> makeRowFn = Nan::New(self->makeRowFn);
  Local<Function> makeObjectRowFn = Nan::New(self->makeObjectRowFn);
  Local<Array> results = Nan::New<Array>();
  Local<Array> errors = Nan::New<Array>();
  bool had_errors = false;

  const char* sql = *sql_str;
  const char* sql_pos = sql;
  size_t sql_len = sql_str.length();
  ...
  while (sql_len) {
    ...
    sqlite3_stmt* stmt;
    const char* new_pos;
    int res = sqlite3_prepare_v3(self->db_,
                                 sql_pos,
                                 sql_len,
                                 prepare_flags,
                                 &stmt,
    ...
    if (res != SQLITE_OK) {
      had_errors = true;
      // TODO: attach code name/value to error object
      Local<Value> err = Nan::Error(sqlite3_errmsg(self->db_));
      Nan::Set(errors, results_pos, err);
      Nan::Set(results, results_pos, rows);
      ++results_pos;
      if (stmt)
        sqlite3_finalize(stmt);
      break;
    ...
    if (stmt) {
      ...
      if (nbinds > 0) {
        if (bind_map) {
          for (int index = 1; index <= nbinds; ++index) {
            ...
            // TODO: switch to map keyed on C string instead to avoid copying
            //       of parameter name?
            auto it = bind_map->find(string(name));
            ...
            if (!bind_value(stmt, index, it->second, &res)) {
              had_errors = true;
              string msg = "Invalid type for bind parameter \"";
              msg += name;
              msg += "\"";
              Local<Value> err = Nan::Error(Nan::New(msg).ToLocalChecked());
              Nan::Set(errors, results_pos, err);
              Nan::Set(results, results_pos, rows);
              ++results_pos;
              sqlite3_finalize(stmt);
          ...
          for (int index = 1;
               index <= nbinds && bind_list_pos < bind_list->size();
               ++index) {
            if (!bind_value(stmt,
                            index,
                            bind_list->at(bind_list_pos++),
                            &res)) {
              had_errors = true;
              string msg = "Invalid type for bind parameter at position ";
              msg += to_string(bind_list_pos - 1);
              Local<Value> err = Nan::Error(Nan::New(msg).ToLocalChecked());
              Nan::Set(errors, results_pos, err);
              Nan::Set(results, results_pos, rows);
              ++results_pos;
      ...
      if (res == SQLITE_ROW) {
        int col_count = sqlite3_data_count(stmt);
        if (col_count) {
          Local<Value> argv[col_count];
          for (int i = 0; i < col_count; ++i) {
            argv[i] = Nan::New<String>(
              sqlite3_column_name(stmt, i)
            ).ToLocalChecked();
          }
          rowFn = Local<Function>::Cast(
            makeObjectRowFn->Call(ctx, rows, col_count, argv).ToLocalChecked()
          );

          uint32_t row_index = 0;
          do {
            vector<RowValue> row(col_count);
            Local<Value> argv[2 + col_count];
            argv[0] = Nan::New<Uint32>(row_index++);
            argv[1] = rowFn;
            for (int i = 0; i < col_count; ++i) {
              switch (sqlite3_column_type(stmt, i)) {
                case SQLITE_NULL:
                ...
                case SQLITE_BLOB: {
                  ...
                  int len = sqlite3_column_bytes(stmt, i);
                  if (len == 0)
                    argv[2 + i] = Nan::NewBuffer(0).ToLocalChecked();
                  else
                    argv[2 + i] = Nan::CopyBuffer(
                ...
                default: {
                  ...
                  int len = sqlite3_column_bytes(stmt, i);
                  if (len == 0) {
                    argv[2 + i] = Nan::EmptyString();
                  } else if (len < EXTERN_APEX) {
                    // V8 makes a copy
                    argv[2 + i] = Nan::New(text, len).ToLocalChecked();
                  } else {
                    void* copy = malloc(len);
                    assert(copy != nullptr);
                    memcpy(copy, text, len);
                    // V8 uses a reference
                    argv[2 + i] = Nan::New(new ExtString(
            ...
            makeRowFn->Call(ctx, rows, 2 + col_count, argv);
          } while ((res = sqlite3_step(stmt)) == SQLITE_ROW);
        } else {
          // No columns thus no row data? Just step until done
          while ((res = sqlite3_step(stmt)) == SQLITE_ROW);
        }
      }
    } else {
    ...
    if (res == SQLITE_DONE) {
      Nan::Set(errors, results_pos, Nan::Null());
    } else {
      had_errors = true;
      // TODO: attach code name/value to error object
      Local<Value> err = Nan::Error(sqlite3_errmsg(self->db_));
  ...
  if (had_errors)
    Nan::Set(ret, 0, errors);
  else
    Nan::Set(ret, 0, Nan::Null());
  Nan::Set(ret, 1, results);
...
NAN_MODULE_INIT(init) {
  ...
  //~ Nan::SetPrototypeMethod(tpl, "querySync", DBHandle::QuerySync);
