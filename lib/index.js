...
const LIMITS_MAX = Object.values(LIMITS).pop();
...
class Database {
  ...
  limit(type, newLimit) {
    if (!Number.isInteger(type))
      throw new TypeError(`Invalid limit type value: ${type}`);
    if (type < 0 || type > LIMITS_MAX)
      throw new RangeError(`Invalid limit type value: ${type}`);
    if (newLimit !== undefined) {
      if (!Number.isInteger(newLimit))
        throw new TypeError(`Invalid new limit value: ${newLimit}`);
      if (newLimit < -(2 ** 31) || newLimit > (2 ** 31 - 1))
        throw new RangeError(`Invalid new limit value: ${newLimit}`);
    } else {
      newLimit = -1;
    }
    return this[kHandle].limit(type, newLimit);
  }
