...
const ABORT_BREAK_TYPE_NONE = 0x00;
const ABORT_BREAK_TYPE_CURRENT = 0x01;
const ABORT_BREAK_TYPE_ALL = 0x02;
...
const kBreakType = Symbol('Iterator break handling');
...
class Statement {
  constructor(db, sqlOrIter, prepareFlags, flags, vals) {
    this[kDatabase] = db;
...
class StatementIterator {
  constructor(abortBreakType, db, sql, prepareFlags, flags, vals) {
    this[kDatabase] = db;
    this[kAborting] = false;
    this[kAborter] = null;
    this[kDone] = false;
    ...
    this[kQueue] = [];
    this[kBreakType] = abortBreakType;
    this[kResume] = false;
    this[kAbortAll] = true;
  }
  ...
  async return() {
    if (this[kBreakType] === ABORT_BREAK_TYPE_ALL) {
      await this.abort();
    } else if (this[kBreakType] === ABORT_BREAK_TYPE_NONE) {
      this[kResume] = true;
      return { done: false };
    }
    return { done: true };
  ...
  if (current) {
    ...
    if (current[kParent]) {
      ...
      if (stmt) {
        ...
        iter[kSlot] = stmt = new Statement(db, iter);
  ...
  if (db[kQueue].length) {
    ...
    if (Array.isArray(current)) {
      ...
      if (iter[kQueue].length) {
        const stmt = new Statement(db, iter);
        iter[kQueue].shift().resolve({ done: false, value: stmt });
      }
    }
...
class Database {
  ...
  queryAsync(sql, opts, vals) {
    ...
    const stmt = new Statement(this, sql, prepareFlags, flags, vals);
  ...
  queryMultiAsync(sql, opts, vals) {
    ...
    let prepareFlags = DEFAULT_PREPARE_FLAGS;
    let flags = 0;
    let abortBreakType = ABORT_BREAK_TYPE_ALL;
    if (Array.isArray(opts)) {
      // query(sql, vals)
      vals = opts;
      opts = undefined;
      ...
      if (typeof opts.prepareFlags === 'number')
        prepareFlags = (opts.prepareFlags & PREPARE_FLAGS_MASK);
      if (opts.abortAllOnBreak === false) {
        if (opts.abortOnBreak === true)
          abortBreakType = ABORT_BREAK_TYPE_CURRENT;
        else
          abortBreakType = ABORT_BREAK_TYPE_NONE;
      }
      if (opts.values !== undefined)
        vals = opts.values;
    }
    ...
    const iter = new StatementIterator(
      abortBreakType, this, sql, prepareFlags, flags, vals
    );
...
function statusCallback(status, lastStmt, data) {
